...
mpz_class final_proof_1;
mpz_class final_proof_2;


//debug proof
vector<mpz_class*> proof_A;
...
Ec1 ped_enc(mpz_class* a, mpz_class* b, mpz_class d, Ec1* ck1, Ec1* ck2, int size){
    ...
    com_ped = com_ped + u*(mie::Vuint(d.get_str().c_str()));

    return com_ped;
}

void prover_set_broadcast_values_and_return_gy(int newsize,Ec1 g1,mpz_class* a,  int deg_A, mpz_class* eval_points, int num_eval){
...
void Prover(Ec2 g2, Ec1 g1,int d,int deg_A, mpz_class* A ,Ec1* v, Ec1* w, mpz_class* eval_points, mpz_class* b, int num_eval,int round){
...
void Verifier(Ec2 g2, Ec1 g1, int d, Ec1* v, Ec1* w, mpz_class* eval_points, Ec1 com, int num_eval, int round){
    ...
    printf("=============Verifier iteration : %d==================\n",round);
        ...
        //=-===============begin degbug====================
        mpz_class* A_prime = proof_A[round];
        mpz_class* b_prime = proof_b[round];
        
  
        mpz_class innerpro = innerMpzMpz(A_prime,b_prime,d_prime);
        innerpro = innerpro%mod;
        mpz_class* A_test = new mpz_class[d];
        mpz_class* A_left = new mpz_class[d_prime];
        mpz_class* A_right = new mpz_class[d_prime];

        for (int i = 0; i < d_prime; ++i)
        {
            A_left[i] = (A_prime[i]*x_inv_mpz)%mod;
            A_right[i] = (A_prime[i]*x_temp)%mod;
            A_test[i] = A_left[i];
        ...
        Ec1 com_test = ped_enc(A_test,b_test,innerpro,v,w,d);
        cout<<"here8"<<endl;
...
int main(int argc, char *argv[])
{
    ...
    prover_set_broadcast_values_and_return_gy(newsize,g1,a,deg_A,eval_points,num_eval_pt);//set com return gy
