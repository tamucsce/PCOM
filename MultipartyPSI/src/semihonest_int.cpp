...
void run_server(std::chrono::seconds accept_time) {
	...
	std::vector<Ec1> enc_poly_sum(2*3);
	// Combined polynomial
        for (auto& msg : msgs) {
	    std::vector<Ec1> enc_poly;
            sock.archive().unpack(msg, enc_poly);
	    DEBUG << "Receiving ..." << std::endl;

	    for(int i = 0; i < enc_poly.size(); i++)
	    {
	       enc_poly_sum[i] = enc_poly_sum[i] + enc_poly[i];
	    }
	}
	sock.broadcast(enc_poly_sum);
...
*/
    ...
    // std::vector<Ec1> pks;
    // pks.emplace_back(g1 * x);
    // pks.emplace_back(g1 * y);
    // pks.emplace_back(g1 * z);
    ...
    */
    ...
    // auto [vec] = sock.receive<std::vector<Ec1>>(10s);
    // ============================================================

    sock.send(Pu, Pv, Ph);
    auto [sum_Pu, sum_Pv, sum_Ph] = sock.receive<Ec1, Ec1, Ec1>(10s);
    ...
    for(int i = 0;i < 2;i++)
    {
	    mpz_class mpzr = rr.get_z_bits(400);
	    mpzr = mpzr % mpr;
	    mpz_class mpzs = rr.get_z_bits(400);
	    mpzs = mpzs % mpr;

	    //char *mStr = "1";
	    mie::Vuint m(i+input);
	    mie::Vuint r(mpzr.get_str());
	    mie::Vuint s(mpzr.get_str());
	    Ec1 Pa = Pu * r;
	    Ec1 Pb = Pv * s;
	    Ec1 Pc = (g1 * (r+s)) + (Ph * m);
	    enc_poly.push_back(Pa);
	    enc_poly.push_back(Pb);
	    enc_poly.push_back(Pc);
	    std::cout << input << "," << i << ":" << Pa << std::endl;
	    std::cout << input <<  "," << i << ":" << Pb << std::endl;
	    std::cout << input <<  "," << i << ":" << Pc << std::endl;
    }
