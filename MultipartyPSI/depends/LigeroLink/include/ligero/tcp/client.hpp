...
#include <ligero/network/analysis.hpp>
...
struct basic_tcp_client : communication_analysis {
...
public:
    ...
    void send(Args&&... args) {
        ...
        /*  Push sent bytes into analysis if no exceptions  */
        this->analysis_emplace_sent(header);
    }
    ...
    void raw_receive(nanoseconds receive_timeout) {
        ...
        co_spawn(executor_,
                 [&]() -> awaitable<void> {
                     ...
                     /*  Push received bytes into analysis  */
                     this->analysis_emplace_received(message_size);
